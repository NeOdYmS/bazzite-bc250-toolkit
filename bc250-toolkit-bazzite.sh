...
cleanup_nct6683_before_nct6687() {
  # BC250 sensor mode selection:
  #   nct6683 = read-only monitoring only
  #   nct6687 = PWM fan control / CoolerControl path
  #
  # Loading both at the same time exposes duplicate nct6686 hwmon devices:
  #   /sys/devices/platform/nct6683.2592
  #   /sys/devices/platform/nct6687.2592
  #
  # For PWM fan control, keep nct6687 and disable the old forced nct6683 setup.
  info "Cleaning conflicting nct6683 read-only configuration before enabling nct6687."

  local f tmp

  f="/etc/modules-load.d/nct6683.conf"
  if [[ -f "$f" ]]; then
    backup_file "$f"
    mv "$f" "${f}.disabled"
    ok "Disabled $f"
  ...
  f="/etc/modprobe.d/sensors.conf"
  if [[ -f "$f" ]] && grep -qE '(^|[[:space:]])nct6683([[:space:]]|$)|options[[:space:]]+nct6683' "$f"; then
    backup_file "$f"
    tmp="$(mktemp)"
    grep -vE '^[[:space:]]*options[[:space:]]+nct6683([[:space:]]|$)|^[[:space:]]*nct6683[[:space:]]*$' "$f" > "$tmp" || true

    if [[ -s "$tmp" ]]; then
      install -m 0644 -o root -g root "$tmp" "$f"
      ok "Removed nct6683 lines from $f"
    else
      mv "$f" "${f}.disabled"
      ok "Disabled nct6683-only $f"
    fi
    rm -f "$tmp"
  fi

  f="/etc/sysconfig/lm_sensors"
  if [[ -f "$f" ]] && grep -q 'nct6683' "$f"; then
    backup_file "$f"
    sed -i 's/\bnct6683\b//g; s/[[:space:]][[:space:]]*/ /g' "$f"
    ok "Removed nct6683 from $f"
  fi

  # Runtime cleanup for the current boot. It is safe if the device/module is absent.
  if [[ -w /sys/bus/platform/drivers/nct6683/unbind && -e /sys/devices/platform/nct6683.2592 ]]; then
    ...
    ok "Unbound runtime platform device nct6683.2592"
  fi

  if lsmod | grep -q '^nct6683 '; then
    modprobe -r nct6683 2>/dev/null || rmmod nct6683 2>/dev/null || warn "Could not unload nct6683 now; reboot will finish cleanup."
  fi
}

verify_single_nct6687_hwmon() {
  local count=0
  local bad=0
  local h name dev modalias

  for h in /sys/class/hwmon/hwmon*; do
    [[ -e "$h/name" ]] || continue
    name="$(cat "$h/name" 2>/dev/null || true)"
    [[ "$name" == "nct6686" ]] || continue

    count=$((count + 1))
    dev="$(readlink -f "$h/device" 2>/dev/null || true)"
    modalias="$(cat "$h/device/modalias" 2>/dev/null || true)"

    echo "$h -> $dev / modalias=$modalias"

    if [[ "$modalias" != "platform:nct6687" ]]; then
      bad=$((bad + 1))
    fi
  done

  if [[ "$count" -eq 1 && "$bad" -eq 0 ]]; then
    ok "Exactly one nct6686 hwmon device is exposed, using nct6687."
    return 0
  fi

  if [[ "$count" -eq 0 ]]; then
    warn "No nct6686 hwmon device currently visible."
    return 1
  fi

  warn "Unexpected nct6686 hwmon state: count=$count, non-nct6687=$bad"
  warn "For PWM control, only platform:nct6687 should remain."
  return 1
}

install_nct6687_sensors() {
  title "NCT6687 Sensors / PWM Fan Control"

  warn "BC250 sensor mode selection:"
  echo "  nct6683 = read-only monitoring only"
  echo "  nct6687 = PWM fan control / CoolerControl path"
  echo
  warn "This step disables old nct6683 forced-loading files to avoid duplicate nct6686 devices."
...
# Prevent duplicate nct6686 hwmon devices:
# nct6683 is read-only monitoring; nct6687 is used for PWM fan control.
if [[ -w /sys/bus/platform/drivers/nct6683/unbind && -e /sys/devices/platform/nct6683.2592 ]]; then
  echo nct6683.2592 > /sys/bus/platform/drivers/nct6683/unbind 2>/dev/null || true
fi
...
EOF_NCT_SERVICE
  ...
  echo
  verify_single_nct6687_hwmon || warn "Reboot may be required to finish nct6683 -> nct6687 cleanup."

  ok "NCT6687 sensor service installed for PWM fan control."
  echo
...
verify_nct6687_sensors() {
  ...
  lsmod | grep -E 'nct6683|nct6687' || warn "No nct6683/nct6687 module is currently loaded."

  echo
  section "nct6686 hwmon devices"
  verify_single_nct6687_hwmon || true

  echo
  section "Conflicting nct6683 configuration"
  if grep -RInE 'nct6683' /etc/modules-load.d /etc/modprobe.d /etc/sysconfig/lm_sensors 2>/dev/null; then
    warn "nct6683 is still referenced in persistent configuration."
    warn "For CoolerControl PWM mode, run NCT6687 Sensors install to disable it."
  else
    ok "No persistent nct6683 reference found in /etc modules/sensors configuration."
  fi
